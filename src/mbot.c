...
bool mbot_loop(repeating_timer_t *rt)
{
    ...
    if (global_comms_status == COMMS_OK)
    {
        ...
        float filtered_linear_vel =  rc_filter_march(&lowpass_linear, mbot_vel_cmd.vx);
        float filtered_angular_vel =  rc_filter_march(&lowpass_angular, mbot_vel_cmd.wz);
        
        // printf("filteredlinear and angular: %f %f\n", filtered_angular_vel, filtered_linear_vel);
        
        // Angular velocity controller 
        // float angular_error = filtered_angular_vel;
        float angular_error = filtered_angular_vel - mbot_vel.wz;
        printf("filtered  and mbot_odometry: %f %f \n", filtered_angular_vel , mbot_vel.wz);
        // printf("mbot.c %f %f \n", mbot_odometry.x, mbot_odometry.y);
        ...
        // Desired velocities using kinematics and linear Angular 
...
{   
    ...
    double kp_right = 0.07;  
    double ki_right= 0.00;    
    double kd_right = 0;   
